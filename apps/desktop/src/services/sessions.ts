...
export interface SessionMessage extends SharedMessage {
  ...
  /**
   * When set, this is a system message that renders an inline peer-evaluation
   * artifact (scorecard + critique graph). The full round payload lives on
   * `DiscussionSession.peerEvalRounds[<id>]` to avoid duplicating the
   * 56-critique blob on the message itself.
   */
  peerEvalRoundId?: string;
}
...
export interface DiscussionSession {
  ...
  /**
   * Peer-evaluation rounds produced at session end. Keyed by round id. Each
   * round contains 8×7 = up to 56 critiques plus per-agent aggregates and
...
function normalizeMessage(input: unknown): SessionMessage | null {
  ...
  return {
    ...
    ...(Array.isArray(record.factCheckBadges)
      ...
      : {}),
    ...(typeof record.peerEvalRoundId === "string" && record.peerEvalRoundId.length > 0
      ? { peerEvalRoundId: record.peerEvalRoundId }
      : {}),
  };
}
...
function normalizePeerEvalScores(input: unknown): PeerEvalScores {
  const record = (input ?? {}) as Partial<Record<keyof PeerEvalScores, unknown>>;
  const dim = (value: unknown): number =>
    Math.max(0, Math.min(100, Math.round(clampNumber(value))));
  return {
    rigor: dim(record.rigor),
    evidence: dim(record.evidence),
    novelty: dim(record.novelty),
    civility: dim(record.civility),
    onTopic: dim(record.onTopic),
  };
}

function normalizePeerEvalStance(value: unknown): PeerEvalStance {
  if (value === "agree" || value === "disagree" || value === "mixed") return value;
  return "mixed";
}

function normalizePeerCritique(input: unknown, agentIds: Set<CouncilAgentId>): PeerCritique | null {
  if (!input || typeof input !== "object") return null;
  const record = input as Partial<PeerCritique>;
  const evaluatorIdRaw = typeof record.evaluatorId === "string" ? record.evaluatorId : "";
  const targetIdRaw = typeof record.targetId === "string" ? record.targetId : "";
  if (!isCouncilAgent(evaluatorIdRaw) || !isCouncilAgent(targetIdRaw)) return null;
  if (!agentIds.has(evaluatorIdRaw) || !agentIds.has(targetIdRaw)) return null;
  if (evaluatorIdRaw === targetIdRaw) return null;
  const critiqueText = typeof record.critique === "string" ? record.critique.trim() : "";
  if (!critiqueText) return null;
  return {
    evaluatorId: evaluatorIdRaw,
    targetId: targetIdRaw,
    scores: normalizePeerEvalScores(record.scores),
    overall: Math.max(0, Math.min(100, Math.round(clampNumber(record.overall)))),
    stance: normalizePeerEvalStance(record.stance),
    critique: critiqueText.slice(0, 1500),
  };
}

function normalizePeerEvalAgentSummary(input: unknown): PeerEvalAgentSummary | null {
  if (!input || typeof input !== "object") return null;
  const record = input as Partial<PeerEvalAgentSummary>;
  const reviewsReceived = Math.max(0, Math.round(clampNumber(record.reviewsReceived)));
  const overallAverage = Math.max(0, Math.min(100, Math.round(clampNumber(record.overallAverage))));
  const rank = Math.max(0, Math.round(clampNumber(record.rank)));
  return {
    averageScores: normalizePeerEvalScores(record.averageScores),
    overallAverage,
    rank,
    reviewsReceived,
    ...(typeof record.standoutCritique === "string" && record.standoutCritique.length > 0
      ? { standoutCritique: record.standoutCritique.slice(0, 500) }
      : {}),
  };
}

function normalizePeerEvalRound(input: unknown): PeerEvalRound | null {
  if (!input || typeof input !== "object") return null;
  const record = input as Partial<PeerEvalRound>;
  const id = cleanText(record.id).trim();
  const topic = cleanText(record.topic).trim();
  if (!id || !topic) return null;
  const generatedAt = clampNumber(record.generatedAt, Date.now());
  const turnsCompleted = Math.max(0, Math.round(clampNumber(record.turnsCompleted)));
  const agentIds = Array.isArray(record.agentIds)
    ? record.agentIds.filter((entry): entry is CouncilAgentId => isCouncilAgent(entry))
    : [];
  if (agentIds.length === 0) return null;
  const agentIdSet = new Set<CouncilAgentId>(agentIds);
  const critiques = Array.isArray(record.critiques)
    ? record.critiques
        .map((entry) => normalizePeerCritique(entry, agentIdSet))
        .filter((entry): entry is PeerCritique => Boolean(entry))
    : [];
  const failedEvaluators = Array.isArray(record.failedEvaluators)
    ? record.failedEvaluators.filter((entry): entry is CouncilAgentId => isCouncilAgent(entry))
    : [];
  const perAgentSummary: Partial<Record<CouncilAgentId, PeerEvalAgentSummary>> = {};
  if (record.perAgentSummary && typeof record.perAgentSummary === "object") {
    for (const [agentId, summary] of Object.entries(
      record.perAgentSummary as Record<string, unknown>,
    )) {
      if (!isCouncilAgent(agentId) || !agentIdSet.has(agentId)) continue;
      const normalized = normalizePeerEvalAgentSummary(summary);
      if (normalized) perAgentSummary[agentId] = normalized;
    }
  }
  return {
    id,
...
function normalizePeerEvalRounds(input: unknown): Record<string, PeerEvalRound> | undefined {
  if (!input || typeof input !== "object") return undefined;
  const out: Record<string, PeerEvalRound> = {};
  for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
    const round = normalizePeerEvalRound(value);
    if (round && round.id === key) {
      out[key] = round;
    }
  }
...
function normalizeDiscussionSession(input: unknown): DiscussionSession | null {
  ...
  return {
    ...
    ...(typeof record.parentMessageId === "string" && record.parentMessageId.length > 0
      ...
      : {}),
    ...(normalizePeerEvalRounds((record as { peerEvalRounds?: unknown }).peerEvalRounds)
      ? {
          peerEvalRounds: normalizePeerEvalRounds(
            (record as { peerEvalRounds?: unknown }).peerEvalRounds,
          ),
        }
      : {}),
  };

...
| `run_command`                            | Opt-in (`all`), terminal client only. A shell command in the workspace under the macOS sandbox (`sandbox-exec`: no network, no keychain or directory-service reads, writes only inside the workspace and never to the workspace folder itself, signals only to its own processes) or bubblewrap on Linux; timeout and output cap. Where no sandbox exists the shell stays off unless you opt in to running it bare, and the record notes an unsandboxed run. |

...
- Select several sessions and delete them together
- Open Settings or the global Command Palette (`⌘K` / `Ctrl+K`)

Type `/` in the composer for commands, listed above the input as you type, the same set the terminal client has: `/council quick|standard|full`, `/deliverable auto|decision|analysis|document|review`, `/review on|off`, `/open <title>`, `/delete <title>`, `/settings`, `/help` and `/quit`. On a session page `/` opens a command line with `/summary`, `/transcript`, `/export`, `/stop`, `/reconvene` and `/home`.

### Session

A session has two pages. The **summary** is built around the product of the run. The bar at the top shows the topic, the phase, the deliverable, the running cost against the estimate, and Stop for a live run. The decision record comes first: the answer with its confidence, the votes, the dissent and why it did not carry, the options considered, what changed during the run, the assumptions, the evidence, the open questions and the next actions, with a copy-as-Markdown button. Then how the debate went, and one analysis panel with four views: the score matrix from the peer review, how the vote split and moved, the critique graph of who rated whom, and the argument map. The side column holds the plan (question, options, participants, lenses, subtasks), the board (settled points, disagreements, evidence, open questions), the convergence judgements, the cost by seat and the hand-off folder.

The **transcript** is one click away: every round with each seat's turn, streamed as it is written, the tool calls the seat made, and its reasoning on request. When the moderator has a clarifying question, or a seat asks to run a tool under an ask-first policy, either page pauses on a prompt.
...
| `run_command`                            | Opt-in (`all`), in both clients. A shell command in the workspace under the macOS sandbox (`sandbox-exec`: no network, no keychain or directory-service reads, writes only inside the workspace and never to the workspace folder itself, signals only to its own processes) or bubblewrap on Linux; timeout and output cap. Where no sandbox exists the shell stays off unless you opt in to running it bare, and the record notes an unsandboxed run. |

The desktop app runs the same sandboxed shell as the terminal. It does not use the macOS App Sandbox, because macOS will not start the command sandbox inside it, and a shell running bare under the app's own sandbox could reach the network and the app's keys. The first launch of a build without the App Sandbox copies the previous build's data out of `~/Library/Containers/com.socratic-council.desktop` and leaves the original in place.
...
**Hand-off.** Every run ends by writing a folder (`<workspace>/handoff`, or `--handoff DIR`): `handoff.md` with the question, the answer, the next steps as a checklist, open questions, dissent and evidence; `record.md`; `document.md` when the deliverable was a document; `board.md`; and `session.json`. Point a person or an agent at it. `socratic-council handoff <session-id>` rebuilds it for any stored session. The folder is never written through a symbolic link, and the brief's workspace listing never follows one, so a link a seat planted cannot redirect it.
...
This builds all workspace packages (`shared` → `core` → apps).

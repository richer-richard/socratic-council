...
The installed desktop app does not offer `run_command`. It runs inside the macOS App Sandbox, macOS will not start the command sandbox inside it, and a shell running bare under the app's own sandbox could reach the network and the app's keys. The Tools card says so, and a council that needs the shell runs from the terminal client.

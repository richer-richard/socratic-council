...
describe("provider request safety", () => {
  ...
  it("uses adaptive thinking (NOT an extended budget) for Claude Opus 4.8", () => {
    // Regression: 4.8 is adaptive-only on the live API — a thinking.type.enabled
    // budget or an explicit temperature both 400. Sending an extended budget here
    // silently fell back to the previous-gen opus-4-7 on every turn.
    const provider = new AnthropicProvider("test-key");
    const request = (
      provider as unknown as {
        buildRequestBody: (...args: unknown[]) => Record<string, unknown>;
...
describe("DeepSeek + Zhipu request shapes (fix 12.1)", () => {
  ...
  it("maps the low reasoning tier to no thinking for Claude Opus 4.8", () => {
    ...
    // Low tier omits thinking entirely (adaptive can't take a budget); 4.8 still
    // prohibits sampling, so temperature is omitted regardless of tier.
    expect(request.thinking).toBeUndefined();
    expect(request.temperature).toBeUndefined();
  });
